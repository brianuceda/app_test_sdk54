...
export default function App() {
  return (
    <SafeAreaView className="flex-1 flex justify-center bg-black/95">
      ...
      <View className="grid grid-cols-2 p-3 gap-3">
        {Array.from({ length: 6 }).map((_, index) => (
          <View
            key={index}
            className="h-32 rounded-md border bg-blue-400/20 ios:bg-green-400/20 border-blue-400/40 ios:border-green-400/40"
          />
        ))}
      </View>
